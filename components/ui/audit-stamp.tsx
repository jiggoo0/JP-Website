...
@version 2026.1.12
@timestamp 2026-01-12T05:25:37.379Z
...
/**
 * 🛰️ UI_PROTOCOL: AUDIT_STAMP_SYSTEM
 * VERSION: 1.2.0
 * ✅ Effect: Physical Impact Animation & Grunge Texture
 */
...
export const AuditStamp = ({ status, date, className }: AuditStampProps) => {
  // 🎨 CONFIG_MAP: กำหนดค่าสีและไอคอนตามสถานะ
  const config = {
    verified: {
      text: 'VERIFIED_OK',
    ...
    pending: {
      ...
      bg: 'bg-amber-50',
      icon: <Check size={24} className="opacity-50" />,
    },
    rejected: {
      text: 'INVALID_ENTRY',
      color: 'text-red-600',
  ...
  return (
    <motion.div
      ...
      className={cn(
        'relative inline-flex flex-col items-center justify-center rounded-sm border-[6px] p-4',
        "before:pointer-events-none before:absolute before:inset-0 before:bg-[url('https://www.transparenttextures.com/patterns/stucco.png')] before:opacity-40",
        current.border,
        current.color,
        current.bg,
        'shadow-[0_0_0_2px_rgba(255,255,255,1),inset_0_0_0_2px_rgba(255,255,255,1)]',
        className,
      )}
    >
      {/* 🛠️ STAMP_CONTENT */}
      <div className="flex flex-col items-center gap-1">
        <div className="flex items-center gap-2">
          {current.icon}
          <span className="text-2xl font-black uppercase italic leading-none tracking-tighter">
            {current.text}
          </span>
        </div>

        <div className="my-1 h-[3px] w-full bg-current opacity-30" />

        <div className="flex flex-col items-center font-mono text-[10px] font-bold uppercase">
          <span>JP_DOCS_AUTHORITY</span>
          {date && <span>AUDIT_DATE: {date}</span>}
        </div>
      </div>

      {/* 🖋️ GRUNGE_OVERLAY: สร้างรอยหมึกไม่สม่ำเสมอ */}
      <div className="pointer-events-none absolute inset-0 bg-[url('https://www.transparenttextures.com/patterns/asfalt-dark.png')] opacity-30 mix-blend-multiply" />
    </motion.div>
  )
}
