...
@version 2026.1.12
@timestamp 2026-01-12T05:25:37.381Z
...
>(({ className, children, ...props }, ref) => (
  <AccordionPrimitive.Header className="flex">
    <AccordionPrimitive.Trigger
      ref={ref}
      className={cn(
        'group flex flex-1 items-center justify-between py-5 text-sm font-black uppercase italic tracking-tight transition-all hover:no-underline [&[data-state=open]>svg]:rotate-180',
        className,
      )}
      {...props}
    >
      {children}
      <div className="bg-slate-50 p-1 transition-colors group-data-[state=open]:bg-[#FCDE09]">
        <ChevronDown className="h-4 w-4 shrink-0 text-[#020617] transition-transform duration-200" />
      </div>
    </AccordionPrimitive.Trigger>
...
>(({ className, children, ...props }, ref) => (
  <AccordionPrimitive.Content
    ref={ref}
    className="overflow-hidden text-sm font-medium leading-relaxed text-slate-600 data-[state=closed]:animate-accordion-up data-[state=open]:animate-accordion-down"
    {...props}
  >
